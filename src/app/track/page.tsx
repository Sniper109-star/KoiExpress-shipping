...
function TrackPageContent() {
  ...
  return <div className="min-h-screen flex flex-col bg-background">
    ...
    <main className="container mx-auto flex-1 px-4 py-8"><div className="mx-auto flex max-w-5xl flex-col gap-6">
      ...
      {shipment && <><div className="grid gap-4 md:grid-cols-3"><div className="rounded-xl border border-primary/30 bg-card p-5 md:col-span-2"><div className="flex items-start justify-between gap-4"><div><p className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-primary">Shipment identity</p><p className="mt-2 font-mono text-xl font-semibold tracking-tight">{shipment.tracking_number}</p></div><span className="rounded-full border border-primary/30 bg-primary/10 px-3 py-1 font-mono text-[10px] font-bold uppercase tracking-wider text-primary">{statusLabel}</span></div><div className="mt-6 flex items-start gap-3"><Truck className="mt-1 size-5 text-primary"/><div><p className="font-semibold">{shipment.origin}</p><p className="text-xs uppercase tracking-wider text-muted-foreground">to {shipment.destination}</p>{lastUpdate && <p className="mt-2 text-xs text-muted-foreground">Last updated {new Date(lastUpdate).toLocaleString()}</p>}</div></div></div><div className="rounded-xl border bg-card p-5"><p className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">Estimated delivery</p><p className="mt-3 font-semibold">{shipment.eta ? new Date(shipment.eta).toLocaleString() : "Being calculated"}</p><p className="mt-2 text-xs text-muted-foreground">Based on the latest shipment event</p></div></div><section className="overflow-hidden rounded-xl border border-primary/30 bg-card shadow-sm"><div className="flex items-center justify-between border-b border-border px-4 py-3"><div><p className="font-mono text-xs font-bold uppercase tracking-[0.2em] text-primary">LIVE SHIPMENT MAP</p><p className="mt-1 text-xs text-muted-foreground">Origin / current location / destination</p></div><span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">MAPTILER</span></div><MapLibreMap branded origin={origin} destination={destination} driverLocation={currentLocation} route={route} markers={markers} className="h-[min(62vw,420px)] min-h-[280px] w-full"/></section><div className="rounded-xl border bg-card p-5"><h2 className="font-semibold">Shipment progress</h2><div className="mt-4 flex flex-col">{stages.map((stage, index) => <div key={stage.status} className="flex gap-3"><div className="flex flex-col items-center"><div className={`flex size-7 items-center justify-center rounded-full border-2 text-xs font-bold ${stage.complete ? "border-primary bg-primary text-primary-foreground" : "border-muted-foreground/30 text-muted-foreground"}`}>{stage.complete ? "✓" : index + 1}</div>{index < stages.length - 1 && <div className={`h-10 w-px ${stage.complete ? "bg-primary/40" : "bg-border"}`} />}</div><div className="pb-5"><p className={`font-medium capitalize ${stage.complete ? "text-foreground" : "text-muted-foreground"}`}>{stage.status.replaceAll("_", " ")}</p><p className="text-sm text-muted-foreground">{stage.event?.message ?? (stage.complete ? "Shipment status updated" : "Pending")}</p>{stage.event && <p className="text-xs text-muted-foreground">{new Date(stage.event.created_at).toLocaleString()}</p>}</div></div>)}</div></div></>}
    </div></main>
  </div>;
}
