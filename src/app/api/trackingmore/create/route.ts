...
export async function POST(request: Request) {
  ...
  const parsed = createTrackingSchema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) {
    return NextResponse.json({ error: "Tracking number and courier code are required." }, { status: 400 });
  }
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: "Tracking provider rejected the request.", details: payload?.meta?.message ?? payload?.message },
