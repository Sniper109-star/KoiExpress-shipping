...
import { z } from "zod"
import { getShippingRates } from "@/lib/services/shipping/karrio-client"
...
export async function POST(request: Request) {
  ...
  try { return NextResponse.json({ rates: await getShippingRates(parsed.data) }) } catch { return NextResponse.json({ error: "Shipping rates are temporarily unavailable." }, { status: 503 }) }
}
