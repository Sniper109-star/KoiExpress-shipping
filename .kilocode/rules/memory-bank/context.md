...
- Customer shipment milestone: `/create-shipment` now presents sender/recipient addresses, optional confirmation email, package weight/dimensions/type, fixed Unifet mock rates (Ground $8.95 / 3–5 days, Priority $14.50 / 2–3 days, Express $24.00 / 1–2 days), service selection, mock payment, test-label creation, canonical tracking link, and optional confirmation through the existing notification route. The custom mock carrier preserves its adapter contract and persists rate metadata for delivery ranges; selected rates continue through authenticated Supabase shipment transitions and label upsert. Typecheck, lint, production build, direct rate-catalog verification, and mobile browser verification pass.
- Checkout/label completion: final review now shows selected service, test cost, ETA range, sender, and recipient before confirmation. The existing label document endpoint persists the generated shipment document and supports both download attachment and inline print mode; the customer flow exposes Download Label, Print Label, and canonical live tracking. Shipment confirmation notifications now include service, ETA, and tracking URL when supplied, while the realtime authenticated shipments history remains the source for status, cost, label, and tracking visibility. Typecheck, lint, production build, and mobile browser verification pass.
- AgentMail-compatible lifecycle notifications: added `notifyShipmentLifecycle` server dispatcher using the existing Resend/AgentMail-compatible `API` provider path. It covers shipment created, service selected, label created, picked up, in transit, out for delivery, delivered, delayed, and exception templates; records notification metadata in Supabase, skips duplicates by shipment/event, and never blocks shipment state changes when email is unavailable or fails. Creation, transition, label, and development simulator routes trigger it directly; Make.com remains outside shipment/tracking/GPS paths. Typecheck, lint, build, and public tracking independence verification pass.
- Billing UI-only milestone: added `/dashboard/billing` with checkout review, display-only payment success/failure states, invoice preview, deterministic invoice numbers, print/download preview actions, payment history, refund status, receipt email preview, and explicit no-charge/Stripe-not-connected messaging. Added Billing to dashboard navigation. No payment provider, webhook, Supabase payment/invoice tables, real invoice persistence, or email calls were added by design.
- Resend delivery milestone: stabilized Supabase/browser client initialization so the production build succeeds with the project environment. The shipment notification dispatcher and direct notification route now use `RESEND_API_KEY` with configurable `RESEND_FROM_EMAIL`, and the scope is narrowed to shipment created, out-for-delivery, and delivered templates. Build, typecheck, lint, and browser smoke checks pass. A real Resend send reached the API but was rejected because the configured sender domain is `gmail.com` and is not verified; delivery will work after `RESEND_FROM_EMAIL` is changed to a verified Resend domain/sender. `example.com` recipients are also rejected by Resend's test restrictions.
- Customer dashboard milestone: rebuilt `/dashboard` as a shipment operations workspace with active, delivered, exceptions, and shipment history filters; cards show tracking number, status, origin/destination, service, cost, ETA, created date, track, view label, and print label actions. It reuses the existing authenticated `/api/shipments` business-scoped query and Supabase Realtime refresh, so customer visibility remains limited to the current business membership. Business organizations/membership schema remains deferred until Supabase schema inspection is available. Typecheck, lint, production build, and mobile browser verification pass.
- Architecture freeze audit: core layers remain Supabase Auth/Database/Realtime, the custom shipping engine with mock carrier/rates/labels/tracking, MapTiler map/proxy routes, and existing Resend/AgentMail-compatible notifications; no new integrations were added. Fixed dashboard View label and Print label actions to open the existing authenticated shipping-label document endpoint instead of a nonexistent page route. Typecheck, lint, production build, unauthenticated API checks, login-route redirect behavior, and public MapTiler tracking surface pass. Authenticated end-to-end shipment creation/rate/confirm/label/live-location/delivery notification testing still requires a confirmed Supabase session and seeded business-owned test shipment; Supabase schema/RLS live inspection remains unavailable.
- Admin foundation slice: `/admin` remains a separate operations portal. Admin login now authenticates through Supabase Auth and requires trusted `admin` or `operations` role from the profile/app metadata path; legacy environment-password/HMAC admin sessions are no longer used by the login flow. `/admin/dashboard` and `/admin/orders` require the Supabase-backed admin role, while `/api/admin/shipments` distinguishes unauthenticated `401` from authenticated non-admin `403` and preserves shipment audit/tracking writes. The overview and shipment management UI remain implemented with the existing Drizzle operational tables. Typecheck, lint, production build, unauthenticated admin API checks, protected dashboard redirect, and mobile admin-login browser verification pass. A valid Supabase admin/operations account is still required to prove the authenticated admin CRUD path.
- Canonical server-generated tracking: added `src/lib/tracking-number.ts`, which generates cryptographically random `UNF#########` codes and canonical tracking URLs. `POST /api/shipments` now assigns and persists the code before returning `trackingNumber` and `trackingUrl`; idempotent retries return the stored code and shipment insertion retries unique collisions. Development test shipments and the Drizzle engine use the same helper, while `/api/shipping/labels` preserves the shipment code rather than replacing it with a mock-carrier code. Typecheck, lint, build, and public tracking route smoke checks pass.
