...
- Email provider routing: AgentMail is now the customer-facing shipment notification provider using the `unifet@agentmail.to` inbox and idempotency keys; Resend remains the admin/support provider for `/api/contact`, using `RESEND_API_KEY` and configurable `RESEND_FROM_EMAIL`. Both keys are present in project environment variables. AgentMail shipment notification dry-run returned HTTP 200, and typecheck, lint, and production build pass; no live customer email was sent during verification.
