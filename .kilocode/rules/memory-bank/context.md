...
- Self-sufficient development tracking flow: added authenticated development-only `POST /api/dev/test-shipment`, which creates a real business-owned shipment with generated `UNF#########` tracking number, NYC/Boston coordinates, ETA, initial in-transit event, and a returned `/track/[trackingNumber]` URL. Dashboard now exposes `Create test shipment` and opens the generated public tracking page; public tracking API now returns only the minimum shipment/event fields needed for customer tracking instead of spreading private database columns. Unauthenticated test endpoint returns 401; typecheck, lint, and production build pass.
